...
constexpr Accessor member()
{
    ...
    } else if constexpr (std::is_const_v<DerivedScope> || !std::is_assignable_v<std::remove_reference_t<T> *, const std::remove_reference_t<T> *> || (is_iterable_v<T> && !std::is_same_v<std::string &, T>)) {
