...
#include "../../keyvalue/scopeiterator.h"
...
namespace Scripting {
    ...
    {
        TypedScopePtr scope = ValueType::fromStack(state, -2).as<TypedScopePtr>();
    ...
    int LuaState::lua_nextScope(lua_State *state)
    {
        ...
        if (lua_isnil(state, -1)) {
            lua_pop(state, 1);
            it = VirtualRange<std::pair<ValueType, ValueType>>(scope).begin();
            ValueType(it).push(state);
        } else if (lua_isuserdata(state, -1)) {
            it = ValueType::fromStack(state, -1).as<KeyValueVirtualIterator>(); //TODO probably a bug: copy instead reference
            ++it;
        }

        if (it != VirtualIteratorEnd{}) {
            return 1 + (*it).second.push(state);
        }
        lua_pop(state, 1);
        lua_pushnil(state);
