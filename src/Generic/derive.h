...
#define DERIVE_FUNCTION2(Name, Fn, ...)                                                                                                                                                        \
    ...
    struct has_function_##Name##_select<T, std::void_t<decltype(Derive_impl_##Name<Args...>::derive(std::declval<T>(), prio {}))>, prio, Args...> : std::true_type {                           \
...
#define DERIVE_OPERATOR(Name, Operator)                                                                                                                      \
    ...
    struct Derive_impl_##Name {                                                                                                                              \
        template <typename T>                                                                                                                                \
        static auto derive(T &&t, Engine::Derive_impl::upToPrio<0>) -> decltype(std::forward<T>(t).operator Operator(std::declval<Arg>()));                  \
                                                                                                                                                             \
        template <typename T>                                                                                                                                \
        static auto derive(T &&t, Engine::Derive_impl::upToPrio<1>) -> decltype(operator Operator(std::forward<T>(t), std::declval<Arg>()));                 \
    };                                                                                                                                                       \
                                                                                                                                                             \
    ...
    struct has_operator_##Name##_select<T, std::void_t<decltype(Derive_impl_##Name<Arg>::derive(std::declval<T>(), prio {}))>, prio, Arg> : std::true_type { \
        using return_type = decltype(Derive_impl_##Name<Arg>::derive(std::declval<T>(), prio {}));                                                           \
    };                                                                                                                                                       \
                                                                                                                                                             \
    template <typename T, typename Arg>                                                                                                                      \
    using has_operator_##Name = has_operator_##Name##_select<T, void, Engine::Derive_impl::upToPrio<1>, Arg>;                                                \
    template <typename T, typename Arg>                                                                                                                      \
