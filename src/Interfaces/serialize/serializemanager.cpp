...
SerializeManager::SerializeManager(const std::string &name)
    : mReceivingMasterState(false), mName(name), mSlaveStreamInvalid(false) {}
...
SerializeManager::~SerializeManager() { clearTopLevelItems(); }
    ...
    if (header.mObject == SERIALIZE_MANAGER) {
        ...
        default:
            throw SerializeException(
                "Unknown Builtin Command-Code for SerializeManager: " +
                std::to_string(header.mCmd));
        }
    } else {
        ...
        case STATE:
            ...
            break;
        default:
            throw SerializeException("Invalid Message-Type: " +
                                     std::to_string(header.mType));
        }
    }
}
...
SerializeManager::getMasterMessageTargets() {
...
bool SerializeManager::addTopLevelItem(TopLevelSerializableUnitBase *unit,
                                       bool sendStateFlag) {
...
void SerializeManager::moveTopLevelItem(TopLevelSerializableUnitBase *oldUnit,
...
StreamError SerializeManager::setSlaveStream(BufferedInOutStream &&stream,
                                             bool receiveState,
                                             TimeOut timeout) {
    if (mSlaveStream)
...
bool SerializeManager::addMasterStream(BufferedInOutStream &&stream,
                                       bool sendStateFlag) {
...
bool SerializeManager::moveMasterStream(ParticipantId streamId,
                                        SerializeManager *target) {
    ...
    try {
        if (mSlaveStream && (&in == &*mSlaveStream)) {
            ...
            if (unit < RESERVED_ID_COUNT) {
                ...
                if (find(mTopLevelUnits.begin(), mTopLevelUnits.end(),
                         u->topLevel()) == mTopLevelUnits.end()) {
                    throw SerializeException(
                        "Illegal Toplevel-Id used! Possible configuration "
                        "mismatch!");
                }
...
bool SerializeManager::receiveMessages(BufferedInOutStream &stream,
    ...
    while (!stream.isClosed() &&
           ((stream.isMessageAvailable() && msgCount == -1) || msgCount > 0)) {
...
bool SerializeManager::sendAllMessages(BufferedInOutStream &stream,
                                       TimeOut timeout) {
...
SerializeManager::getTopLevelUnits() const {
    return mTopLevelUnits;
}

size_t SerializeManager::clientCount() const { return mMasterStreams.size(); }

void SerializeManager::sendState(BufferedInOutStream &stream,
                                 TopLevelSerializableUnitBase *unit) {
