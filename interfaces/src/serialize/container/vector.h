...
namespace Engine {
	namespace Serialize {
		...
		using SerializableVector = VectorImpl<SerializableContainer<container_traits<std::vector, typename UnitHelper<T>::Type>, Creator>>;

		template <class T, const _ContainerPolicy &Config, class Creator = DefaultCreator<>>
		using ObservableVector = ObservableVectorImpl<ObservableContainer<container_traits<std::vector, typename UnitHelper<T>::Type>, Creator, Config>>;
