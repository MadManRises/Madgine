...
namespace Engine {
	namespace Serialize {
		...
		protected:
			...
			iterator read_item_where(const const_iterator &where, SerializeInStream &in, TopLevelSerializableUnitBase *topLevel) {				
				iterator it = this->insert_tuple(where, this->readCreationData(in));				
