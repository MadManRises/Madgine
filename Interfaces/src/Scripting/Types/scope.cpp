...
const Parsing::MethodNode &Scope::getMethod(const std::string &name)
{
    throw ScriptingException(Exceptions::noMethods(getIdentifier()));
}
                                  ...
                                  const ArgumentList &args)
{
    if (!hasScriptMethod(name))
        MADGINE_THROW_NO_TRACE(ScriptingException(
            Exceptions::unknownMethod(name, getIdentifier())));
    if (getArguments(name).size() != args.size()) 
		MADGINE_THROW_NO_TRACE(ScriptingException(
            Exceptions::argumentCountMismatch(getArguments(name).size(), args.size())));
    Stack stack;
