...
const MethodNode &ScriptParser::getGlobalMethod(const std::string &name)
{
    auto it = mGlobalMethods.find(name);
    if (it == mGlobalMethods.end()) {
        throw ScriptingException(Exceptions::noGlobalMethod(name));
    }
    return it->second;
}

const EntityNode &ScriptParser::getEntityDescription(const std::string &name)
{
    auto it = mGlobalTypes.find(name);
    if (it == mGlobalTypes.end()) {
        MADGINE_THROW(ScriptingException(Exceptions::unknownEntityDescription(name)));
    }
    return it->second;
}
...
Prototype & ScriptParser::getPrototype(const std::string & name)
{
	...
	if (it == mPrototypes.end()) {
		MADGINE_THROW(ScriptingException(Exceptions::unknownPrototype(name)));
	}
	return it->second;
}
...
void ScriptParser::parseEntity()
{
    ...
    if (it != mGlobalTypes.end()) {
		if (!mReload)
			throw ScriptingException(Exceptions::doubleTypeDefinition(name));

		node = &it->second;
		node->clear();
	}
	else {
		node = &mGlobalTypes.try_emplace(name, name, mFile).first->second;		
		created = true;
	}
	...
	try {
		...
		Util::UtilMethods::log(e.what(), Util::ERROR_TYPE, traceback);
		if (created) {
...
void ScriptParser::parsePrototype()
{
	...
	try {
		...
		if (it != mPrototypes.end() && !mReload) {
			throw ParseException(Exceptions::doubleTypeDefinition(name));
		}
...
void ScriptParser::parseMethod(EntityNode *entity)
{
    std::string name = consume(NameToken);
    ...
    if (entity) {
		if (entity->hasMethod(name)) {
			throw ParseException(Exceptions::doubleEntityMethod(name, entity->getName()));
		}
    } else {
        auto it = mGlobalMethods.find(name);
        if (it != mGlobalMethods.end()) {
			if (!mReload)
				throw ParseException(Exceptions::doubleGlobalMethodDefinition(name));
			node = &it->second;
		}
	...
	if (!node) {
		...
		created = true;
		if (entity) {
			node = &entity->addMethod(name, mFile);
		}
		else {
			node = &mGlobalMethods.try_emplace(name, name, mFile).first->second;
		}
	}
	else {
		node->clear();
...
std::unique_ptr<const Statements::Statement> ScriptParser::parseFullStatement()
{
    ...
    std::unique_ptr<const Statements::Statement> s = parseTerm();
    ...
    while (loop) {
        ...
        case BinaryLogical: {
            ...
            skipNewlines();
            std::unique_ptr<const Statements::Statement> temp = parseTerm();
...
std::unique_ptr<const Statements::Statement> ScriptParser::parseSingleStatement()
{
	int line = mCurrentLine;
    std::unique_ptr<const Statements::Statement> temp;
    switch (mNextToken) {
    case BracketOpen:
        doRead();
    ...
    case IfToken: {
        ...
        consume(CurlyBracketOpen);
        skipNewlines();
        std::list<std::unique_ptr<const Statements::Statement>> statements, elseStatements;
        while (mNextToken != CurlyBracketClose) {
    ...
    case WhileToken: {
        ...
        consume(CurlyBracketOpen);
        skipNewlines();
        std::list<std::unique_ptr<const Statements::Statement>> statements;
        while (mNextToken != CurlyBracketClose) {
            statements.emplace_back(parseFullStatement());
            skipNewlines();
        }
    ...
    case ForToken: {
        std::unique_ptr<const Statements::Statement> temp2, temp3;
        doRead();
        consume(BracketOpen);
        temp = parseFullStatement();
    ...
    case NameToken: {
        ...
        case BracketOpen: {
            doRead();
            std::list<std::unique_ptr<const Statements::Statement>> args;
            if (mNextToken == BracketClose) {
                doRead();
            } else {
                bool loop = true;
...
std::unique_ptr<const Statements::Statement> ScriptParser::parseTerm()
{
	int line = mCurrentLine;

	std::unique_ptr<const Statements::Statement> s = parseSingleStatement();
	...
	while (loop) {
		...
		case PlusToken: {
			std::string op = consume();
			skipNewlines();
			std::unique_ptr<const Statements::Statement> temp = parseSingleStatement();
...
ValueType ScriptParser::parseConst()
{
	...
	case NullToken:
		...
		return ValueType();
	default:
		throw ParseException(Exceptions::invalidExpression(mCurrentLine));
	}
}
...
std::string ScriptParser::consume(ScriptParser::TokenType expected)
{
    if (mNextToken != expected)
        throw ParseException(Exceptions::unexpectedParseType(typeToString(expected), mFile, mCurrentLine, mStream->tellg() - mLineBase - mNextText.length()));
    return consume();
}
    ...
    if (c == '{') {
        ...
        if (!mStream->eof()){
            if (c == '='){
                ...
                mStream->seekg((int)mStream->tellg() - 1);
            }
        }
    } else if (c == '"') {
        c = safeRead();
