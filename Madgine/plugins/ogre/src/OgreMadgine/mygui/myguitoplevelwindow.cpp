...
namespace Engine
{
	namespace GUI
	{
			...
			mRenderManager(nullptr)
		{
			...
			const auto &settings = system.app().settings();
			if (settings.mUseExternalSettings)
			{
				mWindow = root->createRenderWindow(settings.mWindowName, settings.mWindowWidth, settings.mWindowHeight,
					false, &settings.mWindowParameters);
			}
			else
			{
				mWindow = root->createRenderWindow(settings.mWindowName, settings.mWindowWidth, settings.mWindowHeight, false); // Create Application-Window
			}

			if (settings.mInput) {
				setInput(settings.mInput);
			}
			else {
				mInputHolder = std::make_unique<Input::OISInputHandler>(mWindow);
				setInput(mInputHolder.get());
			}
