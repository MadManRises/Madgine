...
namespace Input {
    ...
    EmscriptenInputHandler::EmscriptenInputHandler(Window::MainWindow &topLevel, Window::OSWindow *window, InputListener *listener)
    ...
    EM_BOOL EmscriptenInputHandler::handleKeyEvent(int eventType, const EmscriptenKeyboardEvent *keyEvent, void *userData)
    {
        ...
        case EMSCRIPTEN_EVENT_KEYDOWN:
            ...
            return _this->injectKeyPress({ static_cast<Key>(keyEvent->keyCode), keyEvent->key[0] });            
        case EMSCRIPTEN_EVENT_KEYUP:
            _this->mKeyDown[keyEvent->keyCode] = false;
            return _this->injectKeyRelease({ static_cast<Key>(keyEvent->keyCode) });            
        }
