...
namespace Tools {
    ...
    bool GuiEditor::init()
    {
        mWindow = root().getGlobalAPIComponent<GUI::GUISystem>(false).topLevelWindows().front().get();
    ...
    void GuiEditor::render()
    {
        ...
        if (ImGui::Begin("GuiEditor")) {
            ImGui::Text("Hovering: %s", ImGui::IsMouseHoveringAnyWindow() ? "true" : "false");
            ImGui::Text("MouseDown: %s", mMouseDown ? "true" : "false");
            ImGui::Text("Dragging: %s", mDragging ? "true" : "false");
            if (mSelected)
                ImGui::Text("Selected - contains: %s", mSelected->widget()->containsPoint(mouse - screenPos.xy(), screenSize, 10.0f) ? "true" : "false");
            ImGui::Text("%f, %f", dragDistance.x, dragDistance.y);
            if (mSelected) {
                ImGui::Text("Selected");
                mSelected->render();
            }
