...
namespace Engine
{
	namespace Tools
	{
		...
		Inspector::Inspector(ImGuiRoot& root) :
			Tool<Inspector>(root)
		{
			Plugins::Plugin plugin(BINARY_FILENAME);
			plugin.load();
			for (const std::experimental::filesystem::path &p : std::experimental::filesystem::directory_iterator(plugin.fullPath().parent_path() / "../Tools/layouts"))
			{
				if (p.extension() != ".xml")
					continue;
