...
namespace Serialize {
    ...
    struct Operations<std::pair<const uint32_t, Scene::Entity::EntityComponentOwningHandle<Scene::Entity::EntityComponentBase>>, Configs...> {
        ...
        {
            Serialize::read(in, *entity.mData->sceneMgr().entityComponentList(p.first).get(p.second.mIndex), name, entity);
        }
        ...
        {
            Serialize::write(out, *entity.mData->sceneMgr().entityComponentList(p.first).get(p.second.mIndex), name, entity);
        }
    };
