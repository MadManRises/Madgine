...
namespace Render {
    ...
    void PointShadowRenderPass::render(Render::RenderTarget *target, size_t iteration)
    {
        ...
        {
            auto perApplication = mPipeline.mapParameters(0).cast<PointShadowPerApplication>();
        ...
        {
            auto perFrame = mPipeline.mapParameters(1).cast<PointShadowPerFrame>();
        ...
        for (std::pair<const std::tuple<const GPUMeshData *, Scene::Entity::Skeleton *>, std::vector<Matrix4>> &instance : instances) {
            ...
            {
                auto perObject = mPipeline.mapParameters(2).cast<PointShadowPerObject>();
