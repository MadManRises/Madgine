...
namespace Widgets {
    ...
    void ExplicitLayoutRenderData::update(const std::vector<ExplicitLayoutConfig> &configs, float pixelSize)
    {
        ...
        std::ranges::transform(
            configs, std::back_inserter(elementConstraints), [&](const ExplicitLayoutConfig &config) {
                SizeConstraints constraints = getConstraints(config, pixelSize);
    ...
    SizeConstraints ExplicitLayoutRenderData::getConstraints(const ExplicitLayoutConfig &config, float pixelSize)
    {
        switch (config.mSizingPolicy) {
        case SizingPolicy::EXPLICIT:
            return { config.mFactor, config.mFactor, config.mFactor };
