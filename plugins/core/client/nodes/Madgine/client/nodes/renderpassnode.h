...
namespace Render {
    ...
    struct MADGINE_CLIENT_NODES_EXPORT RenderPassNode : NodeGraph::Node<RenderPassNode> {
        ...
        virtual void interpretRead(NodeGraph::NodeInterpreterStateBase &interpreter, ValueType &retVal, std::unique_ptr<NodeGraph::NodeInterpreterData> &data, uint32_t providerIndex, uint32_t group = 0) const override;
    };
