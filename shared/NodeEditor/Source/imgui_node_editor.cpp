...
ed::Control ed::EditorContext::BuildControl(bool allowOffscreen)
{
    ...
    auto emitInteractiveArea = [](ObjectId id, const ImRect &rect) {
        ...
        // #debug
        // ImGui::GetWindowDrawList()->AddRectFilled(ImGui::GetItemRectMin(), ImGui::GetItemRectMax(), IM_COL32(255 * !ImGui::IsItemHovered(), 255 * ImGui::IsItemHovered(), 0, 64));
...
void ed::NavigateAnimation::NavigateTo(const ImRect &target, float duration)
{
    ...
    auto epsilon = 1e-4f;
    if (ImFabs(minoffset.x) < epsilon && ImFabs(minoffset.y) < epsilon && ImFabs(maxOffset.x) < epsilon && ImFabs(maxOffset.y) < epsilon) {
        duration = 0;
    }
...
ed::EditorAction::AcceptResult ed::SelectAction::Accept(const Control &control)
{
    ...
    if (control.BackgroundActive && ImGui::IsMouseDragging(0, 1)) {
        ...
        if (clickedObject) {
            // Links and nodes cannot be selected together
            if ((clickedObject->AsLink() && Editor->IsAnyNodeSelected()) || (clickedObject->AsNode() && Editor->IsAnyLinkSelected())) {
                Editor->ClearSelection();
            }
